/*
  ...
  input example for V.A.G. radio remote control protocol library for arduino

  this example read data on remote control bus and decode it on serial console

  usage:
  connect ground of arduino to ground of radio
  connect pin 2 of arduino to remote pin of radio (any pin with hardware interrupt - INTx can be used)
...
void setup() {
  ...
  remote.begin();
  Serial.begin(115200);
  Serial.println("VAR Radio Remote library - read example");
