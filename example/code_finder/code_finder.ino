...
uint8_t h = 0x00;
int get_key();
void _switch(int key);
void lcd_print_hex(int key, bool is_send = false);
...
void loop() {
    ...
    if (key != oldkey) // if keypress is detected
    {
        ...
        if (key != oldkey) {
            oldkey = key;
            _switch(key); //just to be sure
            if (key == 1 || key == 2) { // Long press support only for 'up' and 'down'
                delay(200); // wait for long press time
                while (oldkey == get_key() && oldkey != -1) { //if still pressed, send command
                    _switch(key);
                    delay(50);
                }
            }
...
void lcd_print_hex(int key, bool is_send) {
    lcd.setCursor(0, 1);
    for (int n = 0; n < 16; n++)
        lcd.print(" ");
    ...
    char s[20];
    sprintf(s, "0x%02X", key); // convert to hex with leading 0x

    if (is_send)
        lcd.print(String(s) + " - send");
    else
        lcd.print(s);
}
